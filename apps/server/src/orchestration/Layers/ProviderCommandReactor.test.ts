...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const generateActivityGroupTitle = vi.fn<TextGenerationShape["generateActivityGroupTitle"]>(
      (_) =>
        Effect.fail(
          new TextGenerationError({
  ...
  it("restarts opencode sessions when opencode effort changes", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-opencode-effort-1"),
        threadId: ThreadId.make("thread-1"),
        message: {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-opencode-effort-2"),
        threadId: ThreadId.make("thread-1"),
        message: {
  ...
  it("restarts opencode sessions when runtime mode changes between turns", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.runtime-mode.set",
        commandId: CommandId.make("cmd-opencode-runtime-mode-initial"),
        threadId: ThreadId.make("thread-1"),
        runtimeMode: "full-access",
        createdAt: now,
      }),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-opencode-runtime-mode-turn-1"),
        threadId: ThreadId.make("thread-1"),
        message: {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.runtime-mode.set",
        commandId: CommandId.make("cmd-opencode-runtime-mode-change"),
        threadId: ThreadId.make("thread-1"),
        runtimeMode: "approval-required",
        createdAt: now,
      }),
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-opencode-runtime-mode-turn-2"),
        threadId: ThreadId.make("thread-1"),
        message: {
  ...
  it("reuses opencode sessions when effort and runtime mode are both unchanged", async () => {
    ...
    for (let turn = 1; turn <= 2; turn++) {
      await Effect.runPromise(
        harness.engine.dispatch({
          type: "thread.turn.start",
          commandId: CommandId.make(`cmd-opencode-stable-turn-${turn}`),
          threadId: ThreadId.make("thread-1"),
          message: {
