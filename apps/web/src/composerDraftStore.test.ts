...
describe("composerDraftStore OpenCode draft restoration", () => {
  const threadId = DraftId.make("thread-opencode-draft");
  ...
  it("stores and retrieves an opencode model selection with effort option", () => {
    ...
    expect(
      useComposerDraftStore.getState().draftsByThreadKey[threadId]?.modelSelectionByProvider
        .opencode,
  ...
  it("restores opencode effort from persisted model options via merge", () => {
    ...
    expect(mergedState.draftsByThreadKey[threadId]?.modelSelectionByProvider.opencode).toEqual(
      modelSelection("opencode", "anthropic/claude-opus-4-5", { effort: "medium" }),
    );
    expect(mergedState.draftsByThreadKey[threadId]?.activeProvider).toBe("opencode");
  });
  ...
  it("preserves other provider options when setting opencode model selection", () => {
    ...
    const draft = useComposerDraftStore.getState().draftsByThreadKey[threadId];
