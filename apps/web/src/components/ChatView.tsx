...
type PersistentTerminalLaunchContext = Pick<
  ...
  "cwd" | "worktreePath" | "executionBackend" | "launchProfile"
>;
...
const PersistentThreadTerminalDrawer = memo(function PersistentThreadTerminalDrawer({
  ...
  const storeSetTerminalLaunchProfile = useTerminalStateStore(
    (state) => state.setTerminalLaunchProfile,
  );
  const storeSetTerminalLaunchContext = useTerminalStateStore(
    (state) => state.setTerminalLaunchContext,
  );
  ...
  const effectiveLaunchProfile = launchContext?.launchProfile;
  ...
  const splitTerminal = useCallback(() => {
    ...
    if (effectiveLaunchProfile) {
      storeSetTerminalLaunchProfile(threadRef, terminalId, effectiveLaunchProfile);
    }
    ...
    storeSetTerminalLaunchProfile,
    storeSplitTerminal,
    threadRef,
  ]);

  const createNewTerminal = useCallback(
    (nextLaunchProfile?: TerminalLaunchProfile) => {
      const terminalId = `terminal-${randomUUID()}`;
      if (nextLaunchProfile && cwd) {
        storeSetTerminalLaunchContext(threadRef, {
          cwd,
          worktreePath: effectiveWorktreePath,
          ...(effectiveExecutionBackend ? { executionBackend: effectiveExecutionBackend } : {}),
          launchProfile: nextLaunchProfile,
        });
        storeSetTerminalLaunchProfile(threadRef, terminalId, nextLaunchProfile);
      } else if (effectiveLaunchProfile) {
        storeSetTerminalLaunchProfile(threadRef, terminalId, effectiveLaunchProfile);
      }
  ...
  return (
    <div className={visible ? undefined : "hidden"}>
      <ThreadTerminalDrawer
        ...
        terminalLaunchProfilesById={terminalLaunchProfilesById}
...
export default function ChatView(props: ChatViewProps) {
  ...
  const storeSetTerminalLaunchProfile = useTerminalStateStore((s) => s.setTerminalLaunchProfile);
  ...
  const clearAttachmentPreviewHandoff = useCallback(
    (messageId: MessageId, handoffAttachments?: AttachmentPreviewHandoff) => {
      delete attachmentPreviewPromotionInFlightByMessageIdRef.current[messageId];
      const currentHandoffAttachments =
        handoffAttachments ?? attachmentPreviewHandoffByMessageIdRef.current[messageId] ?? [];
      ...
      for (const attachment of currentHandoffAttachments) {
  ...
  const clearAttachmentPreviewHandoffs = useCallback(() => {
    ...
    for (const handoffAttachments of Object.values(
      attachmentPreviewHandoffByMessageIdRef.current,
    )) {
      for (const attachment of handoffAttachments) {
        revokeBlobPreviewUrl(attachment.previewUrl);
      }
    }
    attachmentPreviewHandoffByMessageIdRef.current = {};
  ...
  const handoffAttachmentPreviews = useCallback(
    (messageId: MessageId, handoffAttachments: AttachmentPreviewHandoff) => {
      if (handoffAttachments.length === 0) return;

      const previousHandoffAttachments =
        attachmentPreviewHandoffByMessageIdRef.current[messageId] ?? [];
      const nextPreviewUrlSet = new Set(
        handoffAttachments.map((attachment) => attachment.previewUrl),
      );
      for (const previousAttachment of previousHandoffAttachments) {
        if (!nextPreviewUrlSet.has(previousAttachment.previewUrl)) {
          revokeBlobPreviewUrl(previousAttachment.previewUrl);
        }
      }
      setAttachmentPreviewHandoffByMessageId((existing) => {
        const next = {
          ...existing,
          [messageId]: handoffAttachments,
        };
        attachmentPreviewHandoffByMessageIdRef.current = next;
        return next;
      });
  ...
  useEffect(() => {
    ...
    for (const [messageId, handoffAttachments] of Object.entries(
      attachmentPreviewHandoffByMessageId,
    )) {
      ...
      if (
        ...
        serverPreviewUrls.length !== handoffAttachments.length ||
      ...
      void preloadServerPreviews
        .then(() => {
          ...
          clearAttachmentPreviewHandoff(messageId as MessageId, handoffAttachments);
        })
        .catch(() => {
          if (!cancelled) {
            delete attachmentPreviewPromotionInFlightByMessageIdRef.current[messageId];
  ...
  const onToggleRightPanel = useCallback(() => {
    if (routeKind === "draft" && draftId) {
      void navigate({
        ...
        search: (previous) => {
          if (rightPanelOpen) {
            return stripRightPanelSearchParams(previous);
          }
          return {
            ...previous,
            rightPanel: "1" as const,
            rightPanelTab:
              previous.rightPanelTab === "files" || previous.rightPanelTab === "diff"
                ? previous.rightPanelTab
                : getStoredRightPanelTab(),
          };
        },
      });
  ...
  const splitTerminal = useCallback(() => {
    ...
    const terminalId = `terminal-${randomUUID()}`;
    if (activeTerminalLaunchContext?.launchProfile) {
      storeSetTerminalLaunchProfile(
        activeThreadRef,
        terminalId,
        activeTerminalLaunchContext.launchProfile,
      );
    }
  ...
  useEffect(() => {
    ...
    for (const removedMessage of removedMessages) {
      const handoffAttachments = collectUserMessageAttachmentPreviewHandoff(removedMessage);
      if (handoffAttachments.length > 0) {
        handoffAttachmentPreviews(removedMessage.id, handoffAttachments);
        continue;
      }
  ...
  const onRemoveQueuedFollowup = useCallback(
    ...
    [environmentId, activeThreadId, setStoreThreadError],
  );
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      {/* Horizontal split: chat column + optional right panel */}
