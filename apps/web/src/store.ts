...
export interface EnvironmentState {
  ...
  queuedFollowupsByThreadId: Record<ThreadId, QueuedFollowup[]>;
...
const EMPTY_QUEUED_FOLLOWUPS: QueuedFollowup[] = [];
...
function mapThread(thread: OrchestrationThread, environmentId: EnvironmentId): Thread {
  return {
    ...
    queuedFollowups: thread.queuedFollowups.map((q) => mapQueuedFollowup(environmentId, q)),
...
function getThread(state: EnvironmentState, threadId: ThreadId): Thread | undefined {
  ...
  return {
    ...
    queuedFollowups: state.queuedFollowupsByThreadId[threadId] ?? EMPTY_QUEUED_FOLLOWUPS,
...
function writeThreadState(
  ...
  if (previousThread?.queuedFollowups !== nextThread.queuedFollowups) {
    nextState = {
      ...nextState,
      queuedFollowupsByThreadId: {
        ...nextState.queuedFollowupsByThreadId,
        [nextThread.id]: nextThread.queuedFollowups,
      },
    };
  }
...
function removeThreadState(state: EnvironmentState, threadId: ThreadId): EnvironmentState {
  ...
  const { [threadId]: _removedQueuedFollowups, ...queuedFollowupsByThreadId } =
...
> {
  ...
  const queuedFollowupsByThreadId: Record<ThreadId, QueuedFollowup[]> = {};
  ...
  for (const thread of threads) {
    ...
    queuedFollowupsByThreadId[thread.id] = thread.queuedFollowups;
...
function applyEnvironmentOrchestrationEvent(
  ...
  switch (event.type) {
    ...
    case "thread.queued-followup-enqueued": {
      return updateThreadState(state, event.payload.threadId, (thread) => {
        const queuedFollowup = mapQueuedFollowup(thread.environmentId, event.payload.followup);
