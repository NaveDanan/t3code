...
function createScopedThreadSelector(
  ...
  let previousQueuedFollowups: QueuedFollowup[] | undefined;
  ...
  return (state) => {
    ...
    const queuedFollowups =
      environmentState.queuedFollowupsByThreadId[threadId] ?? EMPTY_QUEUED_FOLLOWUPS;
    ...
    if (
      ...
      previousQueuedFollowups === queuedFollowups
    ) {
    ...
    previousThread = {
      ...
      queuedFollowups:
        nextThread.queuedFollowups.length === 0
          ? EMPTY_QUEUED_FOLLOWUPS
          : nextThread.queuedFollowups,
    };
    return previousThread;
  };
}
